...
router.get(
  ...
  passport.authenticate("google", {
    failureRedirect: "https://copia-frontend.vercel.app/home?error=google",
    session: false,
  }),
  (req, res) => {
    ...
    if (info?.message === "alreadyExists" || info?.message === "loginWithGoogle") {
      ...
      return res.redirect(
        `https://copia-frontend.vercel.app/home?googleAutoLogin=true&token=${info.token}&email=${info.email}`
      );
    }
    ...
    return res.redirect(
      `https://copia-frontend.vercel.app/home?googleComplete=true&token=${token}&email=${user.email}`
    );
  }
);
